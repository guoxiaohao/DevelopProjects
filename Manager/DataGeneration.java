 ...
 public  ArrayList< SuperArea[]>  readSubfromRealdata( String fileLocation){
	...
	while(j<=lastRowNum){
		...
		Node node=new Node();
			...
			if(dvalue>0){
				double wvalue= worksheet.getRow(j).getCell(nodeweightIndex).getNumericCellValue();
				...
				//	System.out.println("lasthour is"+lasthour);
					sp.setIndex(lastsuper);
	...
	int supercount=spa.size()/(hourcount+1); 
...
/*	for(int k=0;k<spa.size();k++){
	...
	if(spa.get(k)!=null){
		...
		System.out.println("spa is not null"+k);
	}
	}*/
	...
	// System.out.println("spa"+spa.get(i).getIndex()+"day"+day );
...
public  ArrayList< SuperArea[]>  readSubfromRealdataTable( String excelLocation){
	DatatoExcel de=new DatatoExcel();
	...
	try {
		datatable= Table.read().csv(excelLocation );
	} catch (IOException e) {
			e.printStackTrace();
	}
	...
	int weightIndex=datatable.columnIndex("Area Weight");
	int hourIndex=datatable.columnIndex("Hour");
	int demandIndex=datatable.columnIndex("Demand");
	int centerIndex=datatable.columnIndex("Centers");
	int nodeweightIndex=datatable.columnIndex("Node Weight");
	int locationIndex_x=datatable.columnIndex("Location.x");
	int locationIndex_y=datatable.columnIndex("Location.y");
	int RvaluesIndex=datatable.columnIndex("R values");
	int SuperNameIndex=datatable.columnIndex("Super Area");
	...
	ViewGroup h=datatable.groupBy("Hour");
	System.out.println("hour number"+h.size());
	

	for(int p=0;p<h.size();p++){
		Table daytable=h.get(p).asTable();  // for each day how many nodes in total
		
		ViewGroup supertable= daytable.groupBy("Super Area") ;
		List<TemporaryView> supers=supertable.getSubTables();  // how many supers in each day
		
	
		
		SuperArea[] supers_oneday=new SuperArea[supers.size()];
		
		// within each super area is the nodes set
		for(int j=0;j<supers.size();j++){
			
			ArrayList<Node> nodes=new ArrayList<Node>();
			Table nodestable=supers.get(j).asTable();
			double weight=Double.valueOf(nodestable.get(0, weightIndex));
			  int demand= Integer.valueOf(nodestable.get(0,demandIndex )) ;
				
			int superareanum=	 Integer.valueOf(nodestable.get(0,SuperNameIndex )) ;
			 String centerstr=nodestable.get(0, centerIndex);
			 ...
			 String[] cstr=centerstr.split(";");
				...
				SuperArea sup=new SuperArea(center.length);
				sup.setCenters(center);
				sup.setHour(p);
				sup.setWeight(weight);
				sup.setDemand(demand);
				sup.setIndex(superareanum);
				
				System.out.println("super num"+superareanum);
				
				
				
			
				if(demand>0){
			for(int m=0;m<nodestable.rowCount();m++){
				Node node=new Node();
				double lx=Double.valueOf(nodestable.get(m, locationIndex_x));
				double ly=Double.valueOf(nodestable.get(m, locationIndex_y));
				double nodeweight=Double.valueOf(nodestable.get(m, nodeweightIndex));
				String rvaluestr=nodestable.get(m, RvaluesIndex).trim() ;	
				String[] rstr=rvaluestr.split(",");
				System.out.println("rstr:"+rvaluestr+"size:"+rvaluestr.length()+ "rstr:"+rstr.length+
						"hour"+p+"supers"+j+"demand:"+demand);
				int[] r=new int[rstr.length];

				
for(int k=0;k<rstr.length;k++){
	System.out.println("rsk:"+rstr[k]);
	r[k]=Integer.valueOf(rstr[k]);
					}

				node.setLocation(new Location(lx,ly));
				node.setRvalues(r);
